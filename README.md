...
- **閱讀介面**：字級／行距／字體（黑體、明體）／配色（米黃、日、夜）；點畫面翻頁、點中央收起工具列，也可直接捲動。
- **翻頁手勢三選一**（設定面板）：關閉／左右邊緣（左右兩側各 25%）／上下區域（上下各 35%，單手好按）。上下區域是上一頁在上、下一頁在下；兩種模式都是點中間收起工具列。舊版的「點左右邊緣翻頁」開關會自動轉成新設定。
...
文字層 PDF 在閱讀畫面可以按「原版」切到頁面檢視看原始版面（圖表、表格），按「文字」切回來。
兩種檢視的位置用 `doc.pageStart`（每一頁的第一個區塊）互相換算，切過去再切回來會停在同一個位置；
進度一律存文字錨點，所以書架百分比、註解、備份都不受影響。頁面檢視固定寬度、不能縮放，也不朗讀不註解。
...
- **背景／鎖屏無法朗讀**：Web Speech API 在瀏覽器進背景或螢幕關閉時會被暫停，網頁繞不過。只能靠 Wake Lock 維持亮屏播放（系統收回時會自動補回；不用無聲影片備援，因為 iOS 對循環、無音軌的影片不會阻止休眠）；回到前景會自動從被打斷那一句接續。
